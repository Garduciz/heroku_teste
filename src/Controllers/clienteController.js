...
class clientesControllers {
    ...
    static listarCliente = (req, res) => {
        ...
        static listarClientesId = (req, res) => {
                ...
                if(err) {
                res.status(400).send({message: `${err.message} - clientes não localizada.`})
                } else {
    ...
    cliente.save((err) => {
        ...
        if(err) {
            ...
            res.status(201).send({message: "cliente cadastrada com sucesso",
...
static excluirClientes = (req, res) =>{
    ...
    clientes.findByIdAndDelete(id, (err) =>{
        if(!err){
            res.status(200).send({message: `Cliente com ID ${id} removida com sucesso` })
        } else {
